...
class Trie:
    ...
    Every edge going from the father toward its sons is labeled with a different letter of
    the alphabet. The labels on a path starting from the root and ending in a leaf will
    form a string stored in that tree.
    ...
    def all_prefixes(self):
        """All strings in the trie can be recovered by a depth-first scan of the tree."""
        pass
...
class PatriciaTree:
    ...
    from [original paper by Donald R. Morrison] (https://dl.acm.org/citation.cfm?id=321481).
...
class PATTree:
    ...
    A Patricia tree (Morrison 1968; Knuth 1973; Flajolet and Sedgewick 1986; and Gonnet 1988)
    is a digital tree where the individual bits of the keys are used to decide on the branching.
    A zero bit will cause a branch to the left subtree, a one bit will cause a branch to
    the right subtree. Hence Patricia trees are binary digital trees. In addition, Patricia trees
    have in each internal node an indication of which bit of the query is to be used for branching.
    This may be given by an absolute bit position, or by a count of the number of bits to skip.
    This allows internal nodes with single descendants to be eliminated, and thus all internal nodes
    of the tree produce a useful branching, that is, both subtrees are non-null. Patricia trees are
    very similar to compact suffix trees or compact position trees (Aho et al. 1974).

    By using the PAT tree, one can search for a pattern query in a binary search fashion.
    Once we reach the desired node we have to make one final comparison with one of the sistrings
    stored in an external node of the current subtree, to ensure that all the skipped bits coincide.
    If they do not coincide, then the key is not in the tree.

    Important notes:
    - store key values at external nodes
    - internal nodes have no key information, just the skip counter and the pointers to the subtrees
    - leaf/external nodes are references to sistrings
    - bucketing of external nodes is used to decrease memory consumption. The external nodes inside
    a bucket do not have any structure associated with them, and any search that has to be done
    in the bucket has to be done on all the members of the bucket
    ...
    GONNET, G. 1988. "Efficient Searching of Text and Pictures (extended abstract)." Technical Report OED-88-02, Centre for the New OED., University of Waterloo.
...
class SuffixTree:
    ...
    A Suffix Tree is a data-structure that allows many problems on strings (sequences of characters)
    to be solved quickly. If txt=t1t2...ti...tn is a string, then Ti=titi+1...tn is the suffix of txt
    that starts at position i.

    The suffix trie is made by compressing all the suffixes of A1…An – 1 into a trie.
    Two or more common prefixes share a common path from the root of the suffix tree (as in a PATRICIA tree).
...
class SuffixArray:
    ...
    def __init__(self, alphabet: List[Any], mark="$"):
        ...
        self.alphabet_index = {k: v for k, v in zip(self.alphabet, range(len(self.alphabet)))}
    ...
    def stable_sort_cyclic_shifts(self, text: str, shift_length, order: List[int], classes: List[int]) -> List[int]:
    ...
    def update_classes(self, new_order: List[int], classes: List[int], shift_length: int) -> List[int]:
        """
