        ...
        for i, n in enumerate(nums):
            ...
            while factor * factor <= n:
                if n % factor == 0:
                    if factor in factor_index:
                        ufind.unite(i, factor_index[factor])
                    else:
            ...
            if n > 1:
                if n in factor_index:
                    ufind.unite(i, factor_index[n])
                else:
                    factor_index[n] = i
        return ufind.components_count == 1


if __name__ == "__main__":
    print(GreatestCommonDivisor().solve_prime_factorization([2, 3, 6]))
    print(GreatestCommonDivisor().solve_prime_factorization([3, 9, 5]))
    print(GreatestCommonDivisor().solve_prime_factorization([4, 3, 12, 8]))
