...
def next_greater_element(nums1: list[int], nums2: list[int]) -> list[int]:
    ...
    # First step: map out the next greater element for
    # each num in the second array (nums2)
    for num in nums2:
        while stack and stack[-1] < num:
            # current num is the next greater element
            # off all elements in the stack that are smaller the num
            next_greater_map[stack.pop()] = num
        stack.append(num)

    # Second step: build the answer array by iterating
    # over the elements in nums1 and collection the next
    # greated element from the map build in the step 1
    for idx, num in enumerate(nums1):
        answer[idx] = next_greater_map.get(num, -1)
